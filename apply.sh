...
original_name="movie_magic"
original_urlname="movie-magic"
original_description="Awesome movie_magic created by peppajoke" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/movie_magic "${name}"
...
then
    ...
    echo "then run 'movie_magic' or 'python -m movie_magic'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'movie_magic' or 'python -m movie_magic' or 'poetry run movie_magic'"
fi
