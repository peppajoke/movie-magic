...
- 🎯 Entry points to execute your program using `python -m <movie_magic>` or `$ movie_magic` with basic CLI argument parsing.
...
---
# movie_magic Flask Application

Awesome movie_magic created by peppajoke
...
```bash
git clone https://github.com/peppajoke/movie-magic movie_magic
cd movie_magic
make install
```
...
```bash
movie_magic create-db   # run once
movie_magic populate-db  # run once (optional)
movie_magic add-user -u admin -p 1234  # ads a user
movie_magic run
```
